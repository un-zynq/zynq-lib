...
class ZYNQPeer extends EventEmitter {
  ...
  async getStream(opts = {}) {
    ...
    try {
      ...
      this.localConstraints = { video: video, audio: audio };
      return this.localStream;
    } catch (e) {
      this.localConstraints = null;
      return null;
  ...
  _setupDataConnection(conn) {
    ...
    conn.on('data', d => {
      ...
      this._dataQueue.push({ from: peerId, type: 'data', data: d });
      this._scheduleFlush();
    });
    conn.on('open', () => {
    ...
    conn.on('error', e => {
      this.emit('error', { type: 'data', id: peerId, error: e });
      this._connecting.delete(peerId);
    });
  }
  _scheduleFlush() {
  ...
  _handleIncomingCall(call) {
    ...
    this.getStream().then(s => {
      ...
      call.on('stream', stream => this.emit('stream', { from: peerId, stream }));
      call.on('close', () => this._handleMediaConnectionClose(peerId));
      call.on('error', e => this.emit('error', { type: 'media', id: peerId, error: e }));
    });
  }
  ...
  _fullPeerDisconnect(peerId) {
    ...
    this.emit('close', { id: peerId });
    if (wasManual) return;
    if (this.config.autoReconnect) {
      this._attemptReconnect(peerId);
    } else {
      this.emit('disconnect', { id: peerId });
    }
  }
  _attemptReconnect(peerId) {
    ...
    if (attempts >= this.config.maxReconnectAttempts) {
      this.emit('error', { type: 'reconnect_failed', id: peerId });
      this.emit('disconnect', { id: peerId });
      this._reconnectAttempts.delete(peerId);
      return;
    }
    ...
    this.emit('reconnecting', { id: peerId, attempt: attempts, delay: Math.round(delay) });
  ...
  call(id, opts = {}) {
    ...
    this.getStream({ video, audio }).then(stream => {
      if (!stream) return;
      const callObj = this.peer.call(id, stream);
      this._calls.set(id, callObj);
      callObj.on('stream', s => this.emit('stream', { from: id, stream: s }));
      callObj.on('close', () => this._handleMediaConnectionClose(id));
      callObj.on('error', e => this.emit('error', { type: 'media', id: id, error: e }));
    });
    return this;
  }
  disconnect(id) {
