(function(root, factory) {
    ...
    if (typeof ZYNQ === 'undefined' || !ZYNQ.Peer) {
        console.error("ZYNQ Patch Error: ZYNQ Library not found!");
        return;
    }
    ...
    const PatchedPeer = function(config) {
        ...
        peer.call = function(id, options) {
            ...
            if (safety.confirmedPeers.has(id)) {
                return originalCall(id, options || {
                    video: true,
                    audio: true
                });
            } else {
                ...
                window.dispatchEvent(new CustomEvent('handshake:sent', {
                    detail: {
                        to: id
                    }
                }));
        ...
        peer.on('message', ({
            ...
            if (data && data._sys) {
                switch (data.type) {
                    case "REQ":
                        window.dispatchEvent(new CustomEvent('handshake:request', {
                            detail: {
                                from,
                                ts: data.ts
                            }
                    ...
                    case "ACC":
                        ...
                        window.dispatchEvent(new CustomEvent('handshake:accepted', {
                            detail: {
                                from,
                                ts: data.ts
                            }
                        ...
                        break;
                    case "REJ":
                        window.dispatchEvent(new CustomEvent('handshake:rejected', {
                            detail: {
                                from,
                                ts: data.ts
                            }
            ...
            if (data && data.body) {
                window.dispatchEvent(new CustomEvent('secure:message', {
                    detail: {
                        from,
                        text: data.body
                    }
        ...
        peer.on('stream', ({
            ...
            if (!safety.confirmedPeers.has(from)) {
                ...
                window.dispatchEvent(new CustomEvent('secure:violation', {
                    detail: {
                        from
                    }
                }));
            ...
            window.dispatchEvent(new CustomEvent('secure:stream', {
                detail: {
                    from,
                    stream,
        ...
        peer.on('close', ({
            ...
            window.dispatchEvent(new CustomEvent('secure:closed', {
                detail: {
                    id
                }
            }));
